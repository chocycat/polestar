...
export const useCommandSearch = (query: MaybeRef<string>): ComputedRef<SearchResult[]> => {
  query = toRef(query);
 ...
 query.value.length > 0 ?
    ...
    query.value.length > 0 ?
    {
      ...
      component: h(SearchItemAction, {
        ...
        icon: 'ri:global-fill',
        callback: () => {
          window.$electron.spawn(`$BROWSER "${SEARCH_URL(encodeURIComponent(query.value)).replace(/"/g, "\\\"")}"`);
        },
        hintIcon: 'ri:external-link-line'
      })
    } : undefined,
