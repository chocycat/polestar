	...
	window.$electron.onEvent(
		...
		async (
			...
			if (content_type.toLowerCase().includes("string")) {
				...
				const index = clipboard.value.findIndex(
					(x) => x.content?.type === "text" && x.content.text === text,
				);
				if (index !== -1) {
					id = clipboard.value[index]!.id as any;
				...
				const index = clipboard.value.findIndex(
					(x) => x.content?.type === "image" && x.content.src === src,
				);
				if (index !== -1) {
					id = clipboard.value[index]!.id as any;
					clipboard.value.splice(index, 1);
