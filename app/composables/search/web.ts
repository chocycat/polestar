...
export const useWebSearch = (query: MaybeRef<string>, fetch?: ComputedRef<boolean>): Ref<SearchResult[]> => {
  query = toRef(query);
  ...
  watch(query, async () => {
    ...
    try {
      ...
      results.value = [
        {
          id: `builtin/wikipedia+${summary.title}`,
      ...
      languages.forEach(async (l) => {
        ...
        try {
          if (
            results.value.some(
              (x) => x.id === `builtin/wikipedia+${summary.title}+${l}`
            )
          )
            throw 'exists';
          ...
          results.value.push({
            id: `builtin/wikipedia+${summary.title}+${l}`,
