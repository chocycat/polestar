...
import { BrowserWindow, ipcMain, type IpcMainEvent, screen } from "electron";
...
import { search, available } from "./dictionary";
...
export function registerEvents() {
	...
	ipcMain.handle(
		...
		async (
			...
			abortControllers.set(ac, controller);

			try {
				return await download(url, id, output, audio, controller);
			} catch (e) {
	...
	ipcMain.on("abort-signal", (_, signal) => {
		const controller = abortControllers.get(signal);
		if (controller) {
			controller.abort();
			abortControllers.delete(signal);
		}
	});
